...
# This LLM will be used for agents
# model = ChatOpenAI(model_name="gpt-4o", temperature=0)
model = ChatOpenAI(model_name="gpt-4o-mini", temperature=0)

# Roles
analyst = AnalystAgent(model=model)
coder_main = CoderAgent(model=model, flow="basic")
...
def simple_text2code(state: GraphState):
    ...
    obj = json.loads(convert_to_messages(state["messages"])[-1].content)

    requirement = obj["prompt"]

    naivecode = coder_main.implement(function_description=requirement)
    
    # return func_body
    return {"result": naivecode, "obj": obj}
...
def decide_to_block_code(state: GraphState, config):
    ...
    max_retries_block = config.get("configurable", {}).get("max_retries", MAX_RETRIES_BLOCK)
    ...
    if needs_block_flow_verdict == "NO":
        ...
        if retries_block < max_retries_block:
            return "block_code"
        else:
            # return "log"
            return "__end__"
...
def populate_answer(state):
    ...
    return {"solution_before_execution": solution_before_exec}
...
def executor_node(state):
    ...
    return {"solution_after_execution": solution_after_exec,
...
def get_block_correctness_and_explanation(state):
    ...
    corr_chain = BLOCK_DEBUG_REQUEST_PROMPT | llm | StrOutputParser()
...
def code_regen_with_block_feedback(state):
    ...
    refine_chain = CODE_REGEN_WITH_BLOCK_FEEDBACK_PROMPT | llm | StrOutputParser()
